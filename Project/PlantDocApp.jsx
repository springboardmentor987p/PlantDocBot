...
function PlantDocApp() {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4 bg-gray-900">
            <div className="w-full max-w-5xl bg-gray-800 rounded-2xl shadow-2xl p-6 md:p-8 space-y-6">
                {/* Header */}
                <header className="text-center py-4 bg-gray-700 rounded-xl shadow-inner mb-4">
                    <h1 className="text-4xl font-extrabold" style={{ color: "#2563eb" }}>PlantDoc Bot</h1>
                    <p className="text-gray-300 mt-1">Smart Plant Health.</p>
                </header>
                {/* Main grid for upload/result */}
                <div className="main-grid">
                    {/* Upload Box */}
                    <div className="upload-box input-panel">
                        <h2>Upload Image / Enter Symptoms</h2>
                        {/* Tabs */}
                        <div className="flex space-x-2 p-1 bg-gray-100 rounded-lg shadow-sm mb-4">
                            <button
                                onClick={() => setActiveTab('image')}
                                className={`flex-1 py-2 text-sm font-semibold rounded-lg transition duration-200 ${activeTab === 'image' ? 'tab-active' : 'tab-inactive'}`}
                            >
                                Image Upload / Drop
                            </button>
                            <button
                                onClick={() => setActiveTab('text')}
                                className={`flex-1 py-2 text-sm font-semibold rounded-lg transition duration-200 ${activeTab === 'text' ? 'tab-active' : 'tab-inactive'}`}
                            >
                                Text Symptoms
                            </button>
                        </div>
                    ...
                    {/* Result Box */}
                    <div className="result-box output-panel">
